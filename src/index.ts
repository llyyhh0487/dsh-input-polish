  ...
  ctx.effect(() =>
    ctx.webServer.register({
      ...
      handler: async (req: IncomingMessage, res: ServerResponse) => {
        ...
        if (text.trim() === '') return sendJson(res, 400, { ok: false, error: 'empty-input' })

        const llm = ctx.get('llm')
        if (llm === undefined) return sendJson(res, 500, { ok: false, error: 'llm-unavailable' })
        ...
        if (provider === '' || model === '') return sendJson(res, 500, { ok: false, error: 'no-model' })
        ...
        try {
          ...
          for await (const chunk of stream) {
            if (chunk && chunk.type === 'text-delta' && typeof chunk.text === 'string') result += chunk.text
            else if (chunk && chunk.type === 'finish') finish = chunk.reason
          }

          if (finish && finish.kind === 'aborted') return sendJson(res, 500, { ok: false, error: 'aborted' })
          if (finish && finish.kind === 'error') return sendJson(res, 500, { ok: false, error: 'model-error' })

          const polished = result.trim()
          if (polished === '') return sendJson(res, 500, { ok: false, error: 'empty-result' })
